...
test_advanced_buf_limits(void)
{
     ...
     * two BlockedBuf keys xquic's own default is 1 MiB / 8 MiB on a SERVER and
     * unbounded on a client; see src/config.h. */
    mqvpn_config_defaults(&cfg);
    ...
    /* Range: the cap is a TYPE limit, not policy. xquic keeps three of these
     * in a size_t (32-bit on the mips/arm OpenWrt targets) and the window in a
     * uint32_t, so a value past it could only arrive truncated — 2^32 would land as 0,
     * i.e. "unbounded", which is the opposite of what an operator typing a huge number
     * wants. Rejected at the surface (warn-and-continue, field keeps its default),
     * boundary accepted. */
    char over[256];
     ...
     * warning and NOT a load failure. Pinned here because the renderer that
     * will emit these keys does not have an [Advanced] block yet — this is
     * the failure it would produce, and it is silent. */
    p = write_tmp("[Hybrid]\nH3BodyBufPerStream = 262144\n");
    mqvpn_config_defaults(&cfg);
 ...
 * the assignment out. That on its own would turn H3BodyBufPerStream — a key
 * whose entire purpose is to bound memory — into a control an operator can
 * set, that this parser accepts and stores, and that changes nothing: the
 * exact shape of bug these keys were written to remove.
 * mqvpn_config_unsupported_buf_limit() is what closes it, and src/main.c
 * refuses to start on a non-NULL answer.
 *
 * Written so it asserts something real in BOTH builds rather than evaporating
 * in one of them: zero is always accepted (it means "leave xquic's own
 * default alone" and needs no field), and a key that is set is accepted
...
test_buf_limits_refused_when_unsupported(void)
{
     ...
     * xqc_conn_settings_t in the xquic this repository pins, so a build that
     * compiles at all can apply them. Pinned here so that a future #ifdef
     * around them without a matching check cannot pass unnoticed. */
    mqvpn_config_defaults(&cfg);
