...
| `H3BodyBufPerStream` | Buffered HTTP/3 DATA payload one request may hold before mqvpn stops reading its transport stream, letting QUIC flow control backpressure the peer (RFC 9000 §4.1) instead of queueing without limit. Bytes. Real backpressure: nothing is dropped and no connection is closed, and the request resumes when the application drains. Applies to client **and** server, unlike `RecvRateLimit`: on an upload the server is the receiver holding the same buffer. Per request only — there is no connection-wide companion, and the note below says why. Maximum `4294967295`; larger values are rejected with a warning and the key falls back to `0`. Read the three notes under this table before setting it | `0` (unbounded) |
| `BlockedBufPerStream` | Per-stream QPACK decode-blocked buffer. A second queue on the same request that `H3BodyBufPerStream` does **not** cover. **Exceeding it closes the connection** — read the note below before setting it. `0` means 1 MiB on a server and unbounded on a client | `0` |
| `BlockedBufPerConn` | The same, across one connection, and with the same connection-closing enforcement. `0` means 8 MiB on a server and unbounded on a client | `0` |
| `MaxRecvWindow` | Ceiling on the **per-stream** receive window in bytes — both the transport parameters mqvpn advertises and the window auto-tune. Not the connection window (that is `RecvRateLimit`). Lowering it bounds per-stream reassembly memory and the reordering spread the receiver absorbs, at a throughput cost on a long-fat or deeply-queued path. Maximum `4294967295` | `0` (16 MiB, the library default) |
...
**`H3BodyBufPerStream` and `MaxRecvWindow` need an xquic that has the matching setting, and no xquic release has one.** They map to `xqc_conn_settings_t.max_body_buf_per_stream` and `.max_recv_window`, neither of which is in the xquic this repository pins today, nor in any xquic release so far. Until an xquic that carries them is pinned, **these two keys cannot be used**: the build probes for each field, and setting a key whose field is missing is **refused at startup** with an error naming it, rather than accepted and ignored — a bound that silently does nothing is indistinguishable from no bound at all. `0` is always accepted on any build, because `0` means "leave the library's own default alone" and needs no field. `BlockedBufPerStream` and `BlockedBufPerConn` are the two that work with the xquic this repository pins today.

**`BlockedBufPerStream` and `BlockedBufPerConn` do not backpressure — they close the tunnel.** When the QPACK decode-blocked buffer passes either limit, mqvpn's HTTP/3 connection is torn down with `H3_EXCESSIVE_LOAD`, and on this transport that is the whole tunnel: every TCP flow riding it dies at once and the client reconnects. The number you write is not a memory target, it is *how much decode-blocked data a peer is allowed to send you before you drop the tunnel*, and what a peer sends is a property of its encoder, the path's reordering and the transfer rate, not of anything this side controls. (Pausing the stream instead is not available: a QPACK-blocked stream is waiting for encoder instructions that arrive through the same connection-level window pausing it would hold shut.)

*One measurement, and where it comes from:* on a two-path test setup built while developing these keys (virtual links, not a deployment), with both keys set to **1 byte** on client and server — so that a single buffered byte tears the tunnel down — mqvpn carried 3.5 GB over two 60-second transfers across five multiplexed flows at 227.8 and 230.3 Mbit/s with **zero connection closes**. The decode-blocked path was not entered at all. That does not make a limit free, and one test setup on virtual links is not a guarantee: what it says is that the legitimate peak on CONNECT-IP traffic is far below any value you would consider writing, so pick one with orders of magnitude of headroom rather than one sized to a memory budget. Re-measure if the request headers ever stop being the fixed CONNECT set they are today.

**Leaving both at `0` is a choice too, and it is a different one on each side.** `0` gets you the library's internal 1 MiB / 8 MiB on a **server** — so the connection-closing behaviour above is already in force on every server, with or without these keys — and **no bound at all on a client**, where one decode-blocked request can grow without limit exactly the way an unbounded body buffer can.

**Set `BlockedBufPerStream` whenever you set `H3BodyBufPerStream`.** A request blocked on QPACK dynamic-table insertions fills a separate buffer that the body-buf limit does not gate, so the honest peak for one wedged request is `H3BodyBufPerStream + BlockedBufPerStream + one 4 KB chunk`.

**`H3BodyBufPerStream` bounds one request, not one connection, and there is deliberately no `H3BodyBufPerConn`.** A request can only be suspended on bytes its own application is able to free. A connection-wide version would suspend a request holding *zero* bytes because of bytes some other request held; with nothing of its own to drain, that request never resumes, its already-delivered bytes are never read, and the tunnel goes on reporting healthy. (A connection-wide version was built while developing these keys and behaved exactly that way; it is in no xquic release.) So the aggregate across concurrent requests is `backlogged requests x H3BodyBufPerStream` and is not bounded here. In practice that term is small beside the transport's own per-stream reassembly queue — on the same test setup as above, 262,144 bytes of body buffer against 16,655,150 bytes of buffered frames on the same stream at the same instant — so `MaxRecvWindow` is the lever for a connection's total, not this.

**Set `MaxRecvWindow` before, or together with, `H3BodyBufPerStream`.** The receive window and the transport's buffered-frame reassembly cap have to be read together: a window of *W* bytes against an average frame of *L* bytes permits *W/L* buffered frames, and if that exceeds the cap the cap trips first — the peer then sees whole-packet drops and a stalled stream instead of clean flow-control backpressure. Pausing the reader is what makes the window bind, so a body-buf limit added on its own can turn a memory problem into a stall.
