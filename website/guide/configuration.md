...
| `H3BodyBufPerStream` | Buffered HTTP/3 DATA payload one request may hold before mqvpn stops reading its transport stream, letting QUIC flow control backpressure the peer (RFC 9000 &sect;4.1) instead of queueing without limit. Bytes. Applies to client **and** server, unlike `RecvRateLimit`: on an upload the server is the receiver holding the same buffer. Maximum `4294967295`; larger values are rejected with a warning and the key falls back to `0`. Read the two notes under this table before setting it | `0` (unbounded) |
| `H3BodyBufPerConn` | The same budget across every request stream on one connection. Shared, so a value below (concurrent requests &times; `H3BodyBufPerStream`) lets a slow reader pause the others | `0` (unbounded) |
| `BlockedBufPerStream` | Per-stream QPACK decode-blocked buffer. A second queue on the same request that `H3BodyBufPerStream` does **not** cover. `0` means 1 MB on a server and unbounded on a client &mdash; see the note below | `0` |
| `BlockedBufPerConn` | The same, across one connection. `0` means 8 MB on a server and unbounded on a client | `0` |
...
In JSON, the section is an `"advanced"` object with snake_case keys (`recv_rate_limit`, `udp_gso`, `udp_gro`, `h3_body_buf_per_stream`, `h3_body_buf_per_conn`, `blocked_buf_per_stream`, `blocked_buf_per_conn`, `max_recv_window`).

**Set `BlockedBufPerStream` whenever you set `H3BodyBufPerStream`.** A request blocked on QPACK dynamic-table insertions fills a separate buffer that the body-buf limit does not gate, so the honest peak for one wedged request is `H3BodyBufPerStream + BlockedBufPerStream + one 4 KB chunk`. Leaving `BlockedBufPerStream` at `0` gets you the library's internal 1 MB on a server but **no bound at all on a client**, which is where the body-buf limit is most likely to be wanted.
