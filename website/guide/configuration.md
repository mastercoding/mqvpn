...
| `BlockedBufPerStream` | Per-stream QPACK decode-blocked buffer; sets `xqc_conn_settings_t.max_blocked_buf_per_stream`. **Exceeding it closes the connection** — read the note below before setting it. `0` means 1 MiB on a server and unbounded on a client | `0` |
| `BlockedBufPerConn` | The same, across one connection; sets `xqc_conn_settings_t.max_blocked_buf_per_conn`, with the same connection-closing enforcement. `0` means 8 MiB on a server and unbounded on a client | `0` |
| `MaxRecvWindow` | Ceiling on the **per-stream** receive window in bytes; sets `xqc_conn_settings_t.max_recv_window`. Not the connection window (that is `RecvRateLimit`). Maximum `4294967295` | `0` (xquic's own default) |
