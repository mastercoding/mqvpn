...
*Measured, for this transport's own traffic:* with both keys set to **1 byte** on client and server &mdash; so that a single buffered byte tears the tunnel down &mdash; a two-path Starlink+5G rig carried 3.5 GB over two 60-second transfers across five multiplexed flows at 227.8 and 230.3 Mbit/s with **zero connection closes**. The decode-blocked path was not entered at all. That does not make a limit free, and it is one rig on virtual links: it means the legitimate peak on this traffic is far below any value you would consider writing, so pick one with orders of magnitude of headroom rather than one sized to a memory budget. Re-measure if the request headers ever stop being the fixed CONNECT set they are today.

**Leaving both at `0` is a choice too, and it is a different one on each side.** `0` gets you the library's internal 1 MiB / 8 MiB on a **server** &mdash; so the connection-closing behaviour above is already armed on every concentrator, and always has been &mdash; and **no bound at all on a client**, where one decode-blocked request can grow without limit exactly the way an unbounded body buffer can.
...
**`H3BodyBufPerStream` bounds one request, not one connection, and there is deliberately no `H3BodyBufPerConn`.** A request can only be suspended on bytes its own application is able to free. A connection-wide version of this key existed briefly and could suspend a request holding *zero* bytes because of bytes some other request held; with nothing of its own to drain, that request never resumed, its already-delivered bytes were never read, and the tunnel went on reporting healthy. So the aggregate across concurrent requests is `backlogged requests x H3BodyBufPerStream` and is not bounded here. In practice that term is small beside the transport's own per-stream reassembly queue &mdash; 262,144 bytes of body buffer against a measured 16,655,150 bytes of buffered frames on the same stream at the same instant &mdash; so `MaxRecvWindow` is the lever for a connection's total, not this.
