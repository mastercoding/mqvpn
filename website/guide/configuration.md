...
| `BlockedBufPerStream` | Per-stream QPACK decode-blocked buffer. A second queue on the same request that `H3BodyBufPerStream` does **not** cover. **Exceeding it closes the connection** &mdash; read the note below before setting it. `0` means 1 MB on a server and unbounded on a client | `0` |
| `BlockedBufPerConn` | The same, across one connection, and with the same connection-closing enforcement. `0` means 8 MB on a server and unbounded on a client | `0` |
...
**Leaving both at `0` is a choice too, and it is a different one on each side.** `0` gets you the library's internal 1 MB / 8 MB on a **server** &mdash; so the connection-closing behaviour above is already armed on every concentrator, and always has been &mdash; and **no bound at all on a client**, where one decode-blocked request can grow without limit exactly the way an unbounded body buffer can.
