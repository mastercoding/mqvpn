...
**`H3BodyBufPerStream` と `MaxRecvWindow` には対応する設定を持つ xquic が必要です。** この 2 つは `xqc_conn_settings_t.max_body_buf_per_stream` と `.max_recv_window` に対応しますが、これらは 2 つの `BlockedBuf` フィールドより新しいメンバです。ビルド時にそれぞれの有無を検出し、フィールドがない xquic ではこれらのキーは受け付けて無視されるのではなく、キー名を示すエラーとともに**起動時に拒否**されます — 何もしない上限は上限がないのと区別がつかないからです。`0` はどのビルドでも受け付けられます。`0` は「ライブラリ自身の既定値に触らない」という意味で、フィールドを必要としないためです。`BlockedBufPerStream` と `BlockedBufPerConn` はこのリポジトリがこれまで pin してきたすべての xquic で動作します。
...
*測定例と、その出所：* 社内の 2 経路テスト環境（実運用ではなく仮想リンク）で、両方のキーをクライアント・サーバとも **1 バイト** に設定し — つまり 1 バイトでもバッファされたらトンネルが落ちる状態にして — 60 秒の転送 2 回、5 本の多重化されたフローで 227.8 / 230.3 Mbit/s、合計 3.5 GB を**コネクション切断 0 回**で運びました。デコードブロック経路には一度も入りませんでした。これで上限が無害になるわけではなく、仮想リンク 1 環境の結果にすぎません。言えるのは、CONNECT-IP トラフィックの正当なピークは書きたくなるどの値よりもはるか下だということです。メモリ予算から逆算した値ではなく、桁違いの余裕を持たせた値を選んでください。リクエストヘッダが今の固定的な CONNECT セットでなくなったら測り直してください。

**両方を `0` のままにするのも 1 つの選択であり、その意味は左右で異なります。** `0` は**サーバ**ではライブラリ内部の 1 MiB / 8 MiB を意味し — つまり上記の切断動作はすべてのサーバで、昔から既に武装されています — **クライアントでは上限がありません**。クライアントでは 1 つのデコードブロックされたリクエストが、上限のないボディバッファと同じように無制限に肥大化し得ます。
...
**`H3BodyBufPerStream` が拘束するのは 1 リクエストであって 1 コネクションではなく、`H3BodyBufPerConn` は意図的にありません。** リクエストを一時停止できるのは、そのリクエスト自身のアプリケーションが解放できるバイトに対してだけです。コネクション全体版は、*一バイトも持っていない*リクエストを他のリクエストのバイトのせいで一時停止し得ます。自分で排出すべきものを持たないそのリクエストは二度と再開せず、既に届いているバイトも読まれないまま、トンネルは正常を報告し続けます。（開発中に実際に作ってその通りの振る舞いを見ました。公開されている xquic には入っていません。）従って同時実行中のリクエストの合計は `滞留リクエスト数 x H3BodyBufPerStream` であり、ここでは拘束されません。実際にはこの項はトランスポート自身のストリーム単位再組み立てキューに比べて小さく — 上と同じ環境で、同じストリームの同じ瞬間にボディバッファ 262,144 バイトに対してバッファされたフレーム 16,655,150 バイト — コネクション全体のレバーはこれではなく `MaxRecvWindow` です。
