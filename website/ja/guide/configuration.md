...
| `H3BodyBufPerStream` | 1 つのリクエストが保持できる HTTP/3 DATA ペイロードの上限（バイト）。これを超えると mqvpn はそのトランスポートストリームの読み出しを止め、無制限にキューする代わりに QUIC のフロー制御でピアをバックプレッシャさせます（RFC 9000 §4.1）。真のバックプレッシャです — 破棄もコネクションの切断も行われず、アプリケーションが読み進めればリクエストは再開します。`RecvRateLimit` と違いクライアントとサーバの**両方**に適用されます（アップロードではサーバが同じバッファを持つ受信側になるため）。リクエスト単位のみで、コネクション全体版のキーは意図的にありません（理由は表の下の注記参照）。最大値は `4294967295` で、それを超える値は警告とともに拒否され `0` にフォールバックします。設定する前に表の下の 3 つの注記を必ず読んでください | `0`（無制限） |
| `BlockedBufPerStream` | ストリーム単位の QPACK デコードブロックバッファ。同じリクエスト上にあるもう 1 つのキューで、`H3BodyBufPerStream` では**カバーされません**。**超えるとコネクションが切断されます** — 設定する前に下の注記を読んでください。`0` はサーバで 1 MiB、クライアントでは無制限を意味します | `0` |
| `BlockedBufPerConn` | 上と同じものを 1 コネクション全体で集計した上限で、切断の振る舞いも同じです。`0` はサーバで 8 MiB、クライアントでは無制限を意味します | `0` |
| `MaxRecvWindow` | **ストリーム単位**の受信ウィンドウの上限（バイト） — mqvpn が広告するトランスポートパラメータとウィンドウの自動調整の両方にかかります。コネクションレベルのウィンドウではありません（それは `RecvRateLimit`）。下げるとストリーム単位の再組み立てメモリと受信側が吸収する並べ替え幅を抑えられますが、遅延帯域積の大きい経路やキューの深い経路ではスループットが下がります。最大値は `4294967295` | `0`（16 MiB、ライブラリの既定値） |
...
**`H3BodyBufPerStream` と `MaxRecvWindow` には対応する設定を持つ xquic が必要ですが、そのような xquic のリリースはまだありません。** この 2 つは `xqc_conn_settings_t.max_body_buf_per_stream` と `.max_recv_window` に対応しますが、どちらも現在このリポジトリが pin している xquic にはなく、これまでの xquic のどのリリースにもありません。それらを持つ xquic が pin されるまで、**この 2 つのキーは使えません**。ビルド時にそれぞれのフィールドの有無を検出し、フィールドがないキーを設定した場合、そのキーは受け付けられて無視されるのではなく、キー名を示すエラーとともに**起動時に拒否**されます — 何もしない上限は上限がないのと区別がつかないからです。`0` はどのビルドでも受け付けられます。`0` は「ライブラリ自身の既定値に触らない」という意味で、フィールドを必要としないためです。このリポジトリが現在 pin している xquic で実際に効くのは `BlockedBufPerStream` と `BlockedBufPerConn` の 2 つです。

**`BlockedBufPerStream` と `BlockedBufPerConn` はバックプレッシャではありません — トンネルを切断します。** QPACK デコードブロックバッファがどちらかの上限を超えると、mqvpn の HTTP/3 コネクションは `H3_EXCESSIVE_LOAD` で切断されます。このトランスポートではそれはトンネル全体を意味し、乗っている TCP フローが一斉に死に、クライアントは再接続します。ここに書く数字はメモリの目標値ではなく、*トンネルを切るまでにピアが送ってよいデコードブロックデータ量*です。ピアがどれだけ送るかは相手のエンコーダ、経路の並べ替え、転送レートの性質であって、こちら側で制御できるものではありません。（代わりにストリームを一時停止する選択肢はありません。QPACK でブロックされたストリームは、一時停止が閉ざしてしまうのと同じコネクションレベルウィンドウを通って届くエンコーダ命令を待っているからです。）

*測定例と、その出所：* これらのキーの開発中に用意した 2 経路のテスト環境（実運用ではなく仮想リンク）で、両方のキーをクライアント・サーバとも **1 バイト** に設定し — つまり 1 バイトでもバッファされたらトンネルが落ちる状態にして — 60 秒の転送 2 回、5 本の多重化されたフローで 227.8 / 230.3 Mbit/s、合計 3.5 GB を**コネクション切断 0 回**で運びました。デコードブロック経路には一度も入りませんでした。これで上限が無害になるわけではなく、仮想リンクのテスト環境 1 つの結果にすぎません。言えるのは、CONNECT-IP トラフィックの正当なピークは書きたくなるどの値よりもはるか下だということです。メモリ予算から逆算した値ではなく、桁違いの余裕を持たせた値を選んでください。リクエストヘッダが今の固定的な CONNECT セットでなくなったら測り直してください。

**両方を `0` のままにするのも 1 つの選択であり、その意味は左右で異なります。** `0` は**サーバ**ではライブラリ内部の 1 MiB / 8 MiB を意味し — つまり上記の切断動作は、これらのキーを設定するかどうかにかかわらず、すべてのサーバで既に有効です — **クライアントでは上限がありません**。クライアントでは 1 つのデコードブロックされたリクエストが、上限のないボディバッファと同じように無制限に肥大化し得ます。

**`H3BodyBufPerStream` を設定するときは `BlockedBufPerStream` も設定してください。** QPACK 動的テーブルの挿入待ちになったリクエストは、ボディバッファ上限が見ていない別のバッファを埋めます。従って 1 つの詰まったリクエストの正直なピークは `H3BodyBufPerStream + BlockedBufPerStream + 4 KB チャンク 1 つ` です。

**`H3BodyBufPerStream` が拘束するのは 1 リクエストであって 1 コネクションではなく、`H3BodyBufPerConn` は意図的にありません。** リクエストを一時停止できるのは、そのリクエスト自身のアプリケーションが解放できるバイトに対してだけです。コネクション全体版は、*一バイトも持っていない*リクエストを他のリクエストのバイトのせいで一時停止し得ます。自分で排出すべきものを持たないそのリクエストは二度と再開せず、既に届いているバイトも読まれないまま、トンネルは正常を報告し続けます。（コネクション全体版はこれらのキーの開発中に実際に作られ、まさにその通りに振る舞いました。xquic のどのリリースにも入っていません。）従って同時実行中のリクエストの合計は `滞留リクエスト数 x H3BodyBufPerStream` であり、ここでは拘束されません。実際にはこの項はトランスポート自身のストリーム単位再組み立てキューに比べて小さく — 上と同じテスト環境で、同じストリームの同じ瞬間にボディバッファ 262,144 バイトに対してバッファされたフレーム 16,655,150 バイト — コネクション全体のレバーはこれではなく `MaxRecvWindow` です。

**`MaxRecvWindow` は `H3BodyBufPerStream` より先に、または同時に設定してください。** 受信ウィンドウとトランスポート側の再組み立てフレーム上限はセットで考える必要があります。平均フレーム長 *L* バイトに対してウィンドウ *W* バイトは *W/L* 個のバッファフレームを許し、これが上限を超えると先に上限側が発火します — そのときピアから見えるのは、きれいなフロー制御のバックプレッシャではなくパケット丸ごとの破棄とストールです。ウィンドウを効かせるのは読み手を止めることなので、ボディバッファ上限を単体で追加するとメモリの問題がストールの問題に化けることがあります。
