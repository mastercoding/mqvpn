...
mqvpn_build_conn_settings(const mqvpn_conn_settings_input_t *in, xqc_conn_settings_t *out)
{
     ...
     * max_body_buf_per_stream and max_recv_window are newer members of
     * xqc_conn_settings_t than the two blocked-buf ones, and CMake probes
     * for each. Compiling an assignment out is only half the answer — the
     * other half is mqvpn_buf_limits_unsupported() below, which stops
...
/* See the contract on this function in src/buf_limits.h. Order matters only
 * in that the caller prints one name; H3BodyBufPerStream is checked first
 * because it is the one whose absence costs memory rather than throughput. */
const char *
mqvpn_buf_limits_unsupported(const mqvpn_buf_limits_t *bufs)
{
    if (bufs == NULL) {
        return NULL;
    }
#ifndef MQVPN_HAVE_XQC_MAX_BODY_BUF_PER_STREAM
    if (bufs->h3_body_buf_per_stream != 0) {
        return "H3BodyBufPerStream";
    }
#endif
#ifndef MQVPN_HAVE_XQC_MAX_RECV_WINDOW
    if (bufs->max_recv_window != 0) {
