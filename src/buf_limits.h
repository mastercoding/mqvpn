...
typedef struct mqvpn_buf_limits_s {
     ...
     * settings struct with no defaulting. So 0 means 1 MiB / 8 MiB on the
 ...
 *
 * src/main.c calls this once after the config is loaded and refuses to
 * start. There is no public setter for these limits, so the CLI is the only
 * way in and that one call covers every path. */
