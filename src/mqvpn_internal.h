...
struct mqvpn_config_s {
     ...
     * untouched. Both sides, unlike recv_rate_limit above: the server is the
     * receiver for uploads and holds the identical buffers. Consumed by
...
/* Set the four [Advanced] receive-buffer limits on `cfg`. One setter rather
 * than four because both bridges have to carry all of them: the client bridge
 * (src/platform/client_config_bridge.c) AND the server run loop
 * (linux_platform_run_server). These apply to both sides, so a limit that
 * reaches only one leaves the other direction unbounded. Values are passed
 * through as given — the config surface range-checks them against
