...
mqvpn_platform_apply_client_config(mqvpn_config_t *lib_cfg, const mqvpn_client_cfg_t *cfg)
{
    ...
    /* INI [Advanced] buffer-limit bridge. Unconditional: every field is
     * 0-means-"xquic default", so copying zeroes is a no-op and there is no
     * "unset" to distinguish. The server run loop calls the SAME function
     * (linux_platform_run_server) — that is what keeps these five from
