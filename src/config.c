...
static const cfg_key_desc_t cfg_keys[] = {
     ...
     * default alone, so a config naming none of these is unchanged. The cap
     * is a type limit, not policy — see MQVPN_CONFIG_MAX_BUF_LIMIT. Both
     * client and server, unlike RecvRateLimit above. */
 ...
 * repository pins, so a build that compiles at all can apply them.
 * H3BodyBufPerStream is checked first because its absence costs memory rather
 * than throughput. */
const char *
